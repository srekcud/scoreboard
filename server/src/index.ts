...
function debouncedBroadcastFull() {
    if (broadcastTimeout) clearTimeout(broadcastTimeout);
    broadcastTimeout = setTimeout(() => {
        NS.emit("state:full", STATE);
...
function recheckEndConditions() {
    ...
    let shouldStillBeEnded = false;

    switch (STATE.ended.reason) {
        case "penalty":
            shouldStillBeEnded = leftP >= limit || rightP >= limit;
            break;

        case "gap":
            shouldStillBeEnded = !!(STATE.rules.pointGap && gap >= STATE.rules.pointGap);
            break;

        case "target":
            shouldStillBeEnded = !!(STATE.rules.targetScore &&
                (STATE.leftScore >= STATE.rules.targetScore || STATE.rightScore >= STATE.rules.targetScore));
            break;

        case "time":
            shouldStillBeEnded = STATE.timer.remainingMs <= 0;
            break;
    }

    if (!shouldStillBeEnded) {
        STATE.ended = { over: false, winner: null, reason: "time" };
        NS.emit("event:reopened", { message: "Match réouvert après correction" });
        console.log("Match réouvert: les conditions de fin ne sont plus valides");
    }
}
...
NS.on("connection", (socket) => {
    ...
    socket.on("auth:login", ({ role: r, pin }: { role: typeof role; pin?: string }, ack?: Function) => {
        ...
        if (role === "display") {
            authed = true;
            ack?.({ sessionId, role, controller: false });
            socket.emit("state:full", STATE);
            return;
        }
        if (pin === SETTINGS.pin) {
            authed = true;
            ack?.({ sessionId, role, controller: controllerHolder === sessionId });
            socket.emit("state:full", STATE);
        } else {
            ack?.({ error: { code: "AUTH_BAD_PIN" } });
        }
    });
    ...
    socket.on("control:lock:release", (_: {}, ack?: Function) => {
        if (controllerHolder === sessionId) {
            controllerHolder = null;
            heartbeats.delete(sessionId);
            NS.emit("control:lock:status", { holder: null });
        }
        ack?.({ ok: true });
    });

    socket.on("hb:ping", () => {
        if (controllerHolder === sessionId) heartbeats.set(sessionId, now());
    });

    function requireController(ack?: Function) {
        if (!authed || controllerHolder !== sessionId) {
            ack?.({ error: { code: "NOT_CONTROLLER" } });
            return false;
        }
        return true;
    }
    ...
    socket.on("timer:add", ({ ms }: { ms: number }, ack?: Function) => {
        ...
        if (STATE.timer.running) {
            ack?.({ error: { code: "TIMER_RUNNING", message: "Arrêtez le timer avant d'ajouter du temps" } });
            return;
        }
    ...
    socket.on("score:add", ({ side, delta }: { side: Side; delta: 1|2|3|-1 }, ack?: Function) => {
        ...
        if (STATE.timer.running && delta > 0) {
            ack?.({ error: { code: "TIMER_RUNNING", message: "Arrêtez le timer avant d'ajouter des points" } });
            return;
        }

        if (STATE.ended.over && delta > 0) {
            ack?.({ error: { code: "MATCH_ENDED" } });
            return;
        }
    ...
    socket.on("penalty:add", ({ side, cat, delta }: { side: Side; cat: Cat; delta: 1|-1 }, ack?: Function) => {
        ...
        if (STATE.timer.running) {
            ack?.({ error: { code: "TIMER_RUNNING", message: "Arrêtez le timer avant de modifier les pénalités" } });
            return;
        }
        ...
        const key = cat as "C1"|"C2";
        const val = Math.max(0, (p as any)[key] + delta);
        (p as any)[key] = val;
    ...
    socket.on("senshu:set", ({ side }: { side: Side | null }, ack?: Function) => {
        ...
        if (STATE.timer.running) {
            ack?.({ error: { code: "TIMER_RUNNING", message: "Arrêtez le timer avant de modifier le Senshu" } });
            return;
        }
    ...
    socket.on("penaltyVisibility:set", ({ cat, visible }: { cat: Cat; visible: boolean }, ack?: Function) => {
        if (!requireController(ack)) return;
        STATE.penaltyVisibility[cat] = !!visible;
        saveLastMatch(STATE);
        broadcastFull();
        ack?.({ ok: true });
    });
