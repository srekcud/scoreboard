...
<template>
  <div class="w-full h-full bg-black text-white relative overflow-hidden">
    <div class="absolute left-0 top-0 h-full" style="width:12px;background:#b80000"></div>
    <div class="absolute right-0 top-0 h-full" style="width:12px;background:#0048b8"></div>

    <div class="relative w-full" style="height: 7.5rem;">
      <div v-if="state?.senshu==='red'" class="absolute top-0 bottom-0" :style="{ left: '12px', width: '14px', background:'#ffd400' }"></div>
      <div v-if="state?.senshu==='blue'" class="absolute top-0 bottom-0" :style="{ right: '12px', width: '14px', background:'#ffd400' }"></div>
