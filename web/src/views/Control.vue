  ...
  <div class="w-screen h-screen bg-[#0b0f16] text-white p-4 flex flex-col gap-3">
    ...
    <div v-if="!pinOk" class="flex-1 flex items-center justify-center gap-3">
      <input v-model="pin"
             type="password"
             placeholder="PIN"
             class="px-3 py-2 rounded bg-[#111827] border border-[#374151]"
             @keyup.enter="login" />
    ...
    <div v-else class="flex-1 flex flex-col gap-4">
        ...
        <template v-if="s.state?.penaltyVisibility.C1">
          <div class="mt-3 grid grid-cols-3 items-center">
            <div class="flex justify-center gap-2">
              <button class="px-6 py-3 rounded"
                      :disabled="btnDisabledPenalty(1)"
                      :class="btnDis(btnDisabledPenalty(1))"
                      :style="btnStyle('blue', btnDisabledPenalty(1))"
                      @click="pen('blue','C1',1)">+</button>
              <button class="px-6 py-3 rounded"
                      :disabled="btnDisabledPenalty(-1)"
                      :class="btnDis(btnDisabledPenalty(-1))"
                      :style="btnStyle('blue', btnDisabledPenalty(-1))"
            ...
            <div class="flex justify-center gap-2">
              <button class="px-6 py-3 rounded"
                      :disabled="btnDisabledPenalty(1)"
                      :class="btnDis(btnDisabledPenalty(1))"
                      :style="btnStyle('red', btnDisabledPenalty(1))"
                      @click="pen('red','C1',1)">+</button>
              <button class="px-6 py-3 rounded"
                      :disabled="btnDisabledPenalty(-1)"
                      :class="btnDis(btnDisabledPenalty(-1))"
                      :style="btnStyle('red', btnDisabledPenalty(-1))"
      ...
      <div>
        ...
        <div class="mt-2 flex items-center justify-center gap-4">
          <button class="px-6 py-3 rounded"
                  style="background:#0048b8"
                  :disabled="s.state?.timer.running"
                  :class="btnDis(s.state?.timer.running || false)"
                  @click="s.setSenshu('blue')">Bleu</button>
          <button class="px-6 py-3 rounded"
                  style="background:#1f2937"
                  :disabled="s.state?.timer.running"
                  :class="btnDis(s.state?.timer.running || false)"
                  @click="s.setSenshu(null)">Aucune</button>
          <button class="px-6 py-3 rounded"
                  style="background:#b80000"
                  :disabled="s.state?.timer.running"
                  :class="btnDis(s.state?.timer.running || false)"
                  @click="s.setSenshu('red')">Rouge</button>
        </div>
      </div>
        ...
        <div>
          ...
          <div class="mt-2 grid grid-cols-4 gap-2">
            <button v-for="b in btns" :key="'b'+b" class="h-16 rounded"
                    ...
                    :class="btnDis(btnDisabled(b))"
                    :style="btnStyle('blue', btnDisabled(b))"
        ...
        <div>
          ...
          <div class="mt-2 grid grid-cols-4 gap-2">
            <button v-for="b in btns" :key="'r'+b" class="h-16 rounded"
                    :disabled="btnDisabled(b)"
                    :class="btnDis(btnDisabled(b))"
                    :style="btnStyle('red', btnDisabled(b))"
      ...
      <div class="mt-auto pt-2">
        ...
        <div class="mt-2 flex gap-2 items-stretch">
          <button class="h-16 rounded bg-[#374151] flex items-center justify-center basis-[10%]"
                  title="Reset"
                  :disabled="!s.state?.ended.over"
                  :class="btnDis(!s.state?.ended.over || false)"
                  @click="s.reset()">
            <IconReset />
          </button>
...
function label(b: 1 | 2 | 3 | -1) {
  return b > 0 ? `+${b}` : '−1'
}

function act(side: Side, b: 1 | 2 | 3 | -1) {
  s.addScore(side, b)
}

function pen(side: Side, cat: 'C1' | 'C2', d: 1 | -1) {
  s.addPenalty(side, cat, d)
}

function toggle() {
  s.state?.timer.running ? s.stop() : s.start()
}

function add1s() {
  s.socket?.emit('timer:add', { ms: 1000 })
}

function btnDisabled(b: 1 | 2 | 3 | -1): boolean {
  if (b < 0) return false
  if (s.state?.timer.running) return true
  if (s.state?.ended.over) return true
  return false
}

function btnDisabledPenalty(delta: 1 | -1): boolean {
...
function btnStyle(side: 'blue' | 'red', disabled: boolean) {
  if (disabled) {
    return { background: side === 'blue' ? '#0d1a33' : '#531111' }
  } else {
    return { background: side === 'blue' ? '#0048b8' : '#b80000' }
  }
}

function btnDis(disabled: boolean) {
